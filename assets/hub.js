...
async function callPatchApi(url,sm,btn){var origText=btn?btn.textContent:"";if(btn){btn.disabled=true;btn.textContent="🔄 安装中..."}showToast("正在安装增强补丁，请稍候...");try{var res=await api(url,{method:"POST"});var data=await res.json().catch(function(){return{}});if(!res.ok||data.ok===false)throw new Error(data.error||("HTTP "+res.status));state.runtime=data;updateRuntimeNotice();showToast(sm)}catch(e){var prompt="请帮我安装 plugin-hub 的增强补丁（抽屉下拉增强）";var msg="安装失败："+e.message+"\n\n请复制以下提示词到聊天中，AI 会帮你安装：\n"+prompt;try{if(navigator.clipboard){navigator.clipboard.writeText(prompt);showToast("提示词已复制！请粘贴到聊天中发送给 AI")}else{promptUser(msg)}}catch(e2){promptUser(msg)}throw e}finally{if(btn){btn.disabled=false;btn.textContent=origText}}}function promptUser(msg){var ta=document.createElement("textarea");ta.value=msg;ta.style.cssText="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:400px;height:150px;z-index:99999;background:white;border:2px solid #4f46e5;border-radius:8px;padding:12px;font-size:14px;box-shadow:0 8px 24px rgba(0,0,0,.3)";document.body.appendChild(ta);ta.select();setTimeout(function(){document.body.removeChild(ta)},5000)}

document.getElementById("patch-install").addEventListener("click",async function(){var btn=this;try{await callPatchApi(PATCH_INSTALL_URL,"增强补丁已安装，请重启 Hana",btn)}catch(e){showToast("安装失败: "+e.message)}});
document.getElementById("patch-uninstall").addEventListener("click",async function(){var btn=this;try{await callPatchApi(PATCH_UNINSTALL_URL,"增强补丁已卸载，请重启 Hana",btn)}catch(e){showToast("卸载失败: "+e.message)}});
